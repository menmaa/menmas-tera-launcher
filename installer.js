...
const PATCH_HOSTNAME = 'elpides.menmastera.com';
...
async function startInstallation(win, callback) {
    ...
    try {
        ...
        for(let i = 0; i < buildInfo.fileList.length; i++) {
            let filePath = path.join(DOWNLOAD_PATH, buildInfo.fileList[i].name);
            ...
            if(fileSize != buildInfo.fileList[i].size) {
                startingFileIndex = i;
                lastOffset = fileSize;
        ...
        for(let i = startingFileIndex; i < buildInfo.parts; i++) {
            ...
            let fstream = fs.createWriteStream(path.join(DOWNLOAD_PATH, part.name), { flags: 'a' });
