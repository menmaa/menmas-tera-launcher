...
function GetString(str, tokens) {
    switch (str) {
        // connection-manager
        case 'connection-manager/error-ETIMEDOUT-1': return `[Bağlantı] HATA: Oyun sunucularına bağlanılamadı ${tokens.address}:${tokens.port} (Zama aşımı)! Buna sebep olan sorunlar:`;
        case 'connection-manager/error-ETIMEDOUT-2': return '[Bağlantı] - Stabil olmayan bir internet bağlantısı veya IP-Ban';
        case 'connection-manager/error-ETIMEDOUT-3': return '[Bağlantı] - Oyun sunucularında bakım olması durumu';
